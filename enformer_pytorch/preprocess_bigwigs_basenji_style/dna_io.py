...
_IDX_TO_NT = np.array(list("ACGTN"))


def index_to_dna(idx, allow_4_as_N=True):
    """
    Convert a vector (or batch) of one-hot indices to a DNA string (or list of strings).

    Args:
      idx: np.ndarray of shape (L,) with values in {0,1,2,3} (optionally 4 for N),
           or shape (B, L) for a batch.
      allow_4_as_N: if True, value 4 maps to 'N'. If False, raises on any value >3.

    Returns:
      str if shape (L,), else list[str] for shape (B, L).
    """
    idx = np.asarray(idx)
    if idx.ndim == 1:
        if not allow_4_as_N and np.any((idx < 0) | (idx > 3)):
            raise ValueError(
                "Indices must be in {0,1,2,3}. Found values outside range."
            )
        safe = np.clip(idx, 0, 4) if allow_4_as_N else idx
        return "".join(_IDX_TO_NT[safe])
    elif idx.ndim == 2:
        seqs = []
        for row in idx:
            if not allow_4_as_N and np.any((row < 0) | (row > 3)):
                raise ValueError(
                    "Indices must be in {0,1,2,3}. Found values outside range."
                )
            safe = np.clip(row, 0, 4) if allow_4_as_N else row
            seqs.append("".join(_IDX_TO_NT[safe]))
        return seqs
    else:
        raise ValueError(f"idx must have shape (L,) or (B, L); got {idx.shape}")


def index_to_1hot(idx, dtype=bool, allow_4_as_N=True):
    """
    Convert index vector(s) to one-hot array(s).

    Args:
      idx: np.ndarray shape (L,) or (B, L).
      dtype: bool (strict 0/1) or float (e.g., float16/float32).
      allow_4_as_N: if True, index 4 is treated as 'N' (all zeros if dtype=bool,
                    or 0.25 across A/C/G/T if dtype is float).

    Returns:
      np.ndarray shape (L,4) or (B,L,4).
    """
    idx = np.asarray(idx)
    is_batch = idx.ndim == 2
    if idx.ndim == 1:
    ...
    B, L = idx.shape
    X = np.zeros((B, L, 4), dtype=dtype)

    if allow_4_as_N and np.any(idx == 4):
    ...
    # Mask of valid A/C/G/T (0..3)
    valid = (idx >= 0) & (idx <= 3)
    # Flat index fill for valid positions
    if np.any(valid):
        rows, cols = np.where(valid)
        nts = idx[rows, cols].astype(np.int64)
        X[rows, cols, nts] = 1 if np.issubdtype(X.dtype, np.bool_) else 1.0

    return X[0] if not is_batch else X
