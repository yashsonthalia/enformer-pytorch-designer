#### Largely copy pasted from https://github.com/calico/baskerville/blob/63c245fe9a3627577d1b256ac7bea97710f97766/src/baskerville/dataset.py#L252"'
import glob
import json
import sys
import os
...
def main():
    ...
    train_seqs, train_targets = SeqDataset(
        data_dir=data_dir,
        split_label="train",
        batch_size=1,
        mode="eval",
    ...
    val_seqs, val_targets = SeqDataset(
        data_dir=data_dir,
        split_label="valid",
        batch_size=1,
        mode="eval",
    ...
    test_seqs, test_targets = SeqDataset(
        data_dir=data_dir,
        split_label="test",
        batch_size=1,
        mode="eval",
    ...
    np.save(os.path.join(output_dir, "train_seqs.npy"), train_seqs)
    np.save(os.path.join(output_dir, "train_targets.npy"), train_targets)
    np.save(os.path.join(output_dir, "val_seqs.npy"), val_seqs)
    np.save(os.path.join(output_dir, "val_targets.npy"), val_targets)
    np.save(os.path.join(output_dir, "test_seqs.npy"), test_seqs)
